...
public class TerminalWebsocketService {
  ...
  // 각 WebSocket 세션 ID와 현재 경로 매핑
  private final ConcurrentHashMap<String, String> sessionPathMap = new ConcurrentHashMap<>();
   ...
   * @param sessionId WebSocket 세션 ID (WebSocket 연결이 활성 상태인 동안 특정 클라이언트를 식별하는 고유한 식별자)
   * @return 명령어 실행 결과
   */
  @Transactional
  public String executeCommand(Long workspaceId, String command, String sessionId) throws IOException, InterruptedException {
    String currentPath = sessionPathMap.getOrDefault(sessionId, getWorkspaceRootPath(workspaceId));
    ...
    switch (cmd) {
      ...
      case "cd":
        if (parts.length > 1) {
          return cd(workspaceId, parts[1], currentPath, sessionId);
        } else {
   ...
   * @return 이동 결과 메시지
   */
  private String cd(Long workspaceId, String targetPath, String currentPath, String sessionId) {
    if (targetPath.equals("..")) {
      String newPath = resolvePath(currentPath, targetPath);
      sessionPathMap.put(sessionId, newPath);
